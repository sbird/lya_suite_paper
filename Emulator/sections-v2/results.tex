...
\textbf{The spectral index, $n_P$, is $n_P = 0.963 - 0.99$ when using $T_0$ and $n_P > 0.969$ when using only the flux power spectrum, when the $1\sigma$ boundaries hit the upper limit of the prior volume.}
...
\textbf{The upper limit for $n_P$, as well as the $95\%$ lower limit on $A_P$ when including the temperature data, are truncated by the prior volume. The prior volume is an arbitrary choice for our simulation suite, and so our analysis does not rule out $n_P > 1$. In future work we will expand our simulation volume. Constraints on the timing of helium reionization are also affected by our priors, but the redshift limits of our datasets mean that extending the simulation volume may not provide much extra information. For example, the lower limit of $z^{HeII}_f < 2.76$ (with the flux power spectrum at $z < 2.6$ alone) suggests only that helium reionization may have finished after $z=2.6$, the lowest redshift data at which this chain contains constraints.}
...
\textbf{We considered separate constraints on the DLA efficiency at low and high redshift: $\alpha_{DLA}(z < 2.6)$ and $\alpha_{DLA}(z \geq 2.6)$. The high redshift DLA efficiency $\alpha_{DLA}(z \geq 2.6)$ remained consistent with zero. However, the low redshift parameter was negative; $\alpha_{DLA}(z < 2.6) = -0.0134 \pm 0.0074$, indicating false positives in the eBOSS DLA finder at $z < 2.6$. While most cosmological parameters were unchanged, $n_P$ increased (by about $2-\sigma$) to $n_P = 0.916 \pm 0.014$, reducing the internal tension by $1/3$.}
...
We find $n_P \sim 0.97$ and $\sigma_8 \sim 0.73$, \textbf{a several $\sigma$ tension}. Some of the differences between our constraints on $A_P$ and $\sigma_8$ are due to a lever arm effect: for $n_s < 1$ the power spectrum amplitude will be increased on larger scales and measuring $n_s$ will induce a correlation between $n_s$ and $\sigma_8$.
