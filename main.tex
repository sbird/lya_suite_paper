...
We improve on the state of the art in the simulations performed here. These improvements come both from our use of a ``full-physics'' galaxy formation model and from our larger boxes coupled with a high effective resolution. We double the effective resolution of our emulator by using the multi-fidelity technique from \cite{Ho:2022}. With this we are able to model the flux power spectrum over a wide-range of cosmologies at high (percent-level) accuracy.
...
Much of the information in future experiments will come from other statistics. The correlation function between quasars was measured by Ref.~\cite{Slosar:2011}, and has been used to detect the baryon acoustic oscillation (BAO) feature and thus constrain the expansion rate at $z\sim 2.3$ \cite{dSAgathe:2019, Cuceu:2022}. Concurrently, \Lya~tomography surveys with a high sightline density have allowed mapping coherent Mpc-scale overdensities \cite{Lee:CLAMATO, LATIS, Qezlou:2022}. The simulation suite and spectra presented here will allow future comparisons to these summary statistics.
