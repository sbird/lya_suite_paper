...
Figure~\ref{fig:zhefluxpower} shows the effect of the thermal parameters on the flux power spectrum. On the scales measured by BOSS, the thermal parameters have a relatively modest effect, which is why we supplement our analysis with a measurement of the mean IGM temperature derived from high resolution flux power spectrum data. The initial helium reionization redshift $z_\mathrm{Hei}$ shows a very small effect, due to our choices of plotting redshift, which are either before helium reionization starts or after it is almost finished in the default model at $z=2.9$ \spb{Change this}. We are more sensitive to both $z_\mathrm{Hef}$ and $\alpha_q$, which are somewhat degenerate. Decreasing $\alpha_q$ increases the maximum temperature during helium reionization, and
\spb{Why do zhef and alphaq have such a big effect at z=4.4 before he reion starts?} On large scales the scale-dependent signature of patchy helium reionization is visible
