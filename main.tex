...
We generate simulations at specific points in parameter space using a Latin Hypercube design, following the description in \cite{Bird:2019}. Latin Hypercubes are generated at random on a normalised unit cube, and the design which maximises the spread between parameter points is chosen for the final emulator. We ran $30$ simulations in our initial Latin Hypercube design. We supplemented these simulations with $13$ chosen by Bayesian optimisation, as described in \cite{Rogers:2019}. \spb{What was the acquisition function used?} The parameters simulated are shown in Figure~\ref{fig:samples}, which demonstrates that both sets efficiently cover parameter space.
...
\begin{figure*}
\includegraphics[width=0.45\textwidth]{figures/resolution-convergence.pdf}
